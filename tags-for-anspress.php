...
class Tags_For_AnsPress
{
	...
	public function ask_from_tag_field($args, $editing) {
		...
		$tag_field = '<div class="ap-field-tags ap-form-fields">';
			...
			$tag_field .= '<label class="ap-form-label" for="tags">'.__('Tags', 'tags-for-anspress').' <span class="sr-only">(Tapez et appuyez sur Entrer pour ajouter les mots-clés à la liste)</span></label>';
			...
			$tag_field .= '<div data-role="ap-tagsinput" class="ap-tags-input">';
				...
				$tag_field .= '<p id="ap-tags-list-title" tabindex="0" class="sr-only">'.__('Tags list', 'tags-for-anspress').' </p>';
				...
				if(!empty($tag_val) && is_array($tag_val)){
					foreach($tag_val as $tag){
						$tag_field .= '<li class="ap-tagssugg-item"><button role="button" class="ap-tag-remove"><span class="sr-only">'+__( 'Delete Tag', 'tags-for-anspress' )+'"</span> <span class="ap-tag-item-value">'. $tag .'</span><i class="apicon-x"></i></button><input type="hidden" name="tags[]" value="'. $tag .'" /></li>';
					}
				}
				$tag_field .= '</ul>';
